...
const FinanceOption = ({ title, subtitle, features, recommended }) => (
  <div className={`bg-white/5 border p-8 flex flex-col h-full transition-all duration-300 relative ${recommended ? 'border-hd-orange scale-105 z-10 bg-white/10' : 'border-white/10 hover:border-hd-orange hover:bg-white/5'}`}>
    {recommended && (
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-hd-orange text-white text-xs font-bold uppercase py-1 px-3 rounded-full">
            Più Popolare
        </div>
    )}
    <div className="mb-6 text-center">
        <h3 className="text-3xl font-condensed font-bold uppercase text-white mb-2">{title}</h3>
        <p className="text-hd-orange font-bold uppercase text-sm tracking-wider">{subtitle}</p>
    </div>
    ...
    <Link 
        to="/finance/request" 
        className={`w-full text-center border py-3 font-condensed font-bold uppercase transition-all clip-slant ${recommended ? 'bg-hd-orange border-hd-orange text-white hover:bg-white hover:text-black hover:border-white' : 'border-white text-white hover:bg-white hover:text-black'}`}
    >
        Richiedi Info
    </Link>
  </div>
    ...
    return (
        <div className="border-b border-white/10 last:border-0">
            <button 
                ...
                className="w-full py-6 flex justify-between items-center text-left hover:text-hd-orange transition-colors"
            >
                <span className="text-lg font-condensed font-bold uppercase">{question}</span>
                {isOpen ? <Minus size={20} className="text-hd-orange" /> : <Plus size={20} />}
            </button>
            <AnimatePresence>
                {isOpen && (
                    <motion.div 
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-24 items-center">
            <div>
                ...
                <p className="text-gray-400 mt-8 mb-6 text-lg leading-relaxed">
                ...
                <div className="flex gap-4">
                    <Link to="/finance/request" className="bg-hd-orange text-white px-8 py-3 font-bold font-condensed uppercase hover:bg-white hover:text-black transition-all clip-slant">
                        Calcola Rata
                    </Link>
                </div>
            </div>
            <div className="bg-white/5 p-8 border-l-4 border-hd-orange relative overflow-hidden">
                <div className="absolute top-0 right-0 -mr-8 -mt-8 opacity-5">
                    <FileText size={200} />
                </div>
                <h4 className="text-2xl font-condensed font-bold uppercase mb-6 relative z-10">Documenti Necessari</h4>
                <ul className="space-y-4 text-gray-400 relative z-10">
                    <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-hd-orange font-bold">1</span> Documento d'identità valido</li>
                    <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-hd-orange font-bold">2</span> Codice Fiscale</li>
                    <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-hd-orange font-bold">3</span> Ultima busta paga o Modello Unico</li>
                    <li className="flex items-center gap-3"><span className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-hd-orange font-bold">4</span> IBAN per l'addebito</li>
                </ul>
            </div>
        </div>
        ...
        <div className="max-w-3xl mx-auto">
            <div className="text-center mb-12">
                <HelpCircle size={48} className="text-hd-orange mx-auto mb-4" />
                <h2 className="text-4xl font-condensed font-bold uppercase text-white">Domande Frequenti</h2>
            </div>
